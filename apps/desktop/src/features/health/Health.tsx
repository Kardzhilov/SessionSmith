...
import type { HealthCheck, HealthCheckState, HealthReport } from "../../api/types";
      ...
      <section className="health-section" aria-labelledby="health-checks-heading">
        ...
        <div className="health-checks">
          {report.checks.map((check) => (
            <HealthCheckRow check={check} key={check.id} onRemedy={onRemedy} />
          ))}
        </div>
      </section>
...
function HealthCheckRow({
  ...
  const remedyLabel = check.remedy ? remedyLabels[check.remedy] ?? "Open guide" : null;

  return (
    <article className={`health-check health-check--${check.state}`}>
      <span className="health-status" role="img" aria-label={stateLabel}>
      ...
      {check.remedy && remedyLabel ? (
        <button className="button button--quiet health-check__remedy" type="button" onClick={() => onRemedy(check.remedy!)}>
          {remedyLabel}
        </button>
      ) : (
...
const remedyLabels: Record<string, string> = {
  "install-ffmpeg": "Setup guide",
  "install-uv": "Setup guide",
  "open-backend-settings": "Campaign settings",
  "open-health-docs": "Health guide",
  "open-models": "Models",
};
