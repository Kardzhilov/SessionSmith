...
pub(crate) fn speaker_review(campaign_id: String, stem: String) -> Result<SpeakerReview, String> {
    ...
    for sample in speakers::detect_samples(&text) {
        samples.entry(sample.label).or_default().push(sample.text);
    }
...
fn artifact_path(paths: &CampaignPaths, stem: &str, artifact: Artifact, candidate: bool) -> PathBuf {
    paths
        .notes
        .join(stem)
    ...
    let mut saved_artifacts = fs::read_dir(notes_dir)
        ...
        .filter_map(|entry| {
            ...
            let artifact = ALL_ARTIFACTS
                .iter()
                .copied()
                .find(|artifact| candidates::is_alternate_filename(artifact.filename(), &filename))?;
...
mod tests {
    use super::{
        ...
        validate_artifact_write, validate_transcript_query, CampaignPaths,
    };
    ...
    fn saved_artifacts_only_accept_known_alternate_filenames() {
        ...
        assert!(saved_artifact_path(&paths, "session-1", Artifact::Summary, "summary-alt.md").is_ok());
        assert!(saved_artifact_path(&paths, "session-1", Artifact::Summary, "recap-alt.md").is_err());
        assert!(saved_artifact_path(&paths, "session-1", Artifact::Summary, "../summary-alt.md").is_err());
        assert_eq!(
    ...
    fn provenance_display_text_is_bounded_and_path_free() {
        assert_eq!(sanitize_provenance_text("  engine\nname\u{0000}  ", 20), "engine name");
    ...
    fn transcript_filter_is_bounded_and_printable() {
        assert_eq!(validate_transcript_query(Some("  Alice  ".into())).unwrap(), Some("Alice".into()));
